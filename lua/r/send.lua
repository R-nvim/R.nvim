...
M.set_send_cmd_fun = function()
	...
	elseif
		(type(config.external_term) == "boolean" and config.external_term) or type(config.external_term) == "string"
	then
...
M.GetSourceArgs = function(e)
	...
	local sargs = ""
	if config.get_config().source_args ~= "" then sargs = ", " .. config.get_config().source_args end
...
M.above_lines = function()
	...
	local result = table.concat(vim.tbl_filter(function(line) return line ~= "" end, lines), "\n")
