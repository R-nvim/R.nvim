...
local do_common_global = function()
    ...
    local first_line = "Last change in this file: 2024-03-02"
    if
    ...
    if need_readme then
        ...
        vim.fn.delete(config.compldir .. "/inst_libs")

        local flist = vim.fn.split(vim.fn.glob(config.compldir .. "/objls_*"), "\n")
        flist = vim.fn.extend(
            flist,
            vim.fn.split(vim.fn.glob(config.compldir .. "/args_*"), "\n")
        )

        -- TODO: Remove fun_* and omnils_* from the list after some time since we no longer build them.
        flist = vim.fn.extend(
            flist,
        ...
        flist = vim.fn.extend(
            flist,
            vim.fn.split(vim.fn.glob(config.compldir .. "/fun_*"), "\n")
        )
        ...
        local readme = {
            ...
            "The objls_ and args_ are used for auto completion, the ",
            ", and the ",
            "inst_libs for library description in the Object Browser. If you delete ",
            "them, they will be regenerated.",
            "",
            ...
            "",
            "If you delete this README file, all objls_ and args_ files will be ",
            "regenerated.",
            "",
