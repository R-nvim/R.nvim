...
.onLoad <- function(libname, pkgname) {
  if (Sys.getenv("RNVIM_TMPDIR") == "") return(invisible(NULL))
  library.dynam("nvimcom", pkgname, libname, local = FALSE)

  if (is.null(getOption("nvimcom.verbose"))) options(nvimcom.verbose = 0L)

  # The remaining options are set by Neovim. Don't try to set them in your
  # ~/.Rprofile because they will be overridden here:
  if (
    file.exists(paste0(Sys.getenv("RNVIM_TMPDIR"), "/start_options_utf8.R"))
  ) {
    source(
      paste0(Sys.getenv("RNVIM_TMPDIR"), "/start_options_utf8.R"),
      encoding = "UTF-8"
    )
  } else if (
    file.exists(paste0(Sys.getenv("RNVIM_TMPDIR"), "/start_options.R"))
  ) {
    ...
    options(nvimcom.autoglbenv = 0L)
    options(nvimcom.debug_r = TRUE)
    options(nvimcom.nvimpager = TRUE)
    options(nvimcom.max_depth = 12L)
    options(nvimcom.max_size = 1000000L)
    options(nvimcom.max_time = 100L)
    options(nvimcom.delim = "\t")
  }
  if (getOption("nvimcom.nvimpager")) options(pager = nvim.hmsg)
}
...
run_tcb <- function(...) {
  if (!NvimcomEnv$tcb) return(invisible(FALSE))
  .C(nvimcom_task)
...
set_running_info <- function() {
  ...
  hascolor <- FALSE
  if (length(find.package("colorout", quiet = TRUE, verbose = FALSE)) > 0L)
  ...
  info <- paste0(
    ...
    getOption("continue"),
    "', has_color = ",
    ifelse(hascolor, "true", "false"),
    ", kitty_wid = '",
    Sys.getenv("KITTY_WINDOW_ID"),
    "'",
    ", wez_pane = '",
    Sys.getenv("WEZTERM_PANE"),
    "'",
    ", tmux_pane = '",
