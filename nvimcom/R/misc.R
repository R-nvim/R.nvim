...
nvim.edit <- function(name, file, title) {
  ...
  editf <- paste0(
    ...
    Sys.getenv("RNVIM_ID"),
    "_",
    round(runif(1L, min = 100L, max = 999L))
  )
...
nvim_viewobj <- function(
  ...
  if (is.data.frame(oname) || is.matrix(oname)) {
    ...
    oname_split <- unlist(strsplit(oname_split, "]]", fixed = TRUE))
    ok <- try(o <- get(oname_split[[1L]], envir = .GlobalEnv), silent = TRUE)
    if (length(oname_split) > 1L) {
      for (i in 2L:length(oname_split)) {
...
nvim.list.args <- function(ff) {
  ...
  if (class(mm)[1L] == "MethodsFunction" && length(mm) > 0L) {
...
nvim.getmethod <- function(fname, objclass) {
  if (
    exists(fname, where = 1L) &&
      is.function(get(fname, pos = 1L)) &&
    ...
    for (obc in objclass) {
      ...
      if (length(idx) == 1L) {
        ...
        if (is.function(fun)) {
          ...
          luastr <- paste0(names(luatbl), unname(luatbl), collapse = clpstr)
          luastr <- paste0(
            "{label = '",
            luastr,
            "', cls = 'a', env = '",
...
nvim_complete_args <- function(
  ...
  if (length(grep(paste0("^", rkeyword, "$"), objects(.GlobalEnv))) == 1L) {
    args <- nvim.args(rkeyword, txt = argkey)
    args <- gsub("\005$", "", args)
    argsl <- strsplit(args, "\005")[[1L]]
    argsl <- sub("\004.*", "", argsl)
    args <- paste0(
      "{label = '",
  ...
  if (firstobj != "" && exists(firstobj, where = 1L)) {
