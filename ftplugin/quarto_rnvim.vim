...
let g:Rcfg.quarto_preview_args = get(g:, 'R_quarto_preview_args', '')
let g:Rcfg.quarto_render_args = get(g:, 'R_quarto_render_args', '')

function! RQuarto(what)
    if a:what == "render"
        update
        call g:SendCmdToR('quarto::quarto_render("' . substitute(expand('%'), '\\', '/', 'g') . '"' . g:Rcfg.quarto_render_args . ')')
    elseif a:what == "preview"
        update
        call g:SendCmdToR('quarto::quarto_preview("' . substitute(expand('%'), '\\', '/', 'g') . '"' . g:Rcfg.quarto_preview_args . ')')
    else
