...
function! RMakePDFrrst()
    ...
    call RSetWD()
    if s:has_rst2pdf == 0
        if has_key(g:Rcfg, "rst2pdfpath") && executable(g:Rcfg.rst2pdfpath)
            let s:has_rst2pdf = 1
    ...
    if has_key(g:Rcfg, "rrstcompiler")
        let pdfcmd = pdfcmd . ", compiler='" . g:Rcfg.rrstcompiler . "'"
    endif
    if has_key(g:Rcfg, "knitargs")
        let pdfcmd = pdfcmd . ", " . g:Rcfg.knitargs
    endif
    if has_key(g:Rcfg, "rst2pdfpath")
        let pdfcmd = pdfcmd . ", rst2pdfpath='" . g:Rcfg.rst2pdfpath . "'"
    endif
    if has_key(g:Rcfg, "rst2pdfargs")
        let pdfcmd = pdfcmd . ", " . g:Rcfg.rst2pdfargs
    endif
    let pdfcmd = pdfcmd . ")"
...
let g:Rcfg.strict_rst = get(g:, "R_strict_rst",         1)
