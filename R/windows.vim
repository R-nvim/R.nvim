...
if has_key(g:Rcfg, 'R_pcfg.th')
    let s:rpath = split(g:Rcfg.R_path, ';')
...
function StartR_Windows()
    ...
    if g:rplugin.R =~? 'Rterm' && g:rplugin.R_app =~? 'Rterm'
    ...
    call SetRHome()
    call system("start " . g:rplugin.R . ' ' . join(g:Rcfg.R_args))
    call UnsetRHome()
