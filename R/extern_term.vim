...
let g:Rcfg.silent_term = get(g:, "R_silent_term", 0)
...
if type(g:Rcfg.external_term) == v:t_string
    let s:term_name = substitute(g:Rcfg.external_term, ' .*', '', '')
    let s:term_cmd = g:Rcfg.external_term
    if g:Rcfg.external_term =~ ' '
...
if !exists("s:term_name")
    call RWarningMsg("Please, set the option 'external_term' in your init.lua. See the plugin documentation for details.")
