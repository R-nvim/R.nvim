...
let g:Rcfg.external_term = 1
let g:Rcfg.applescript = 0
let g:rplugin.tmux_split = 1
let g:Rcfg.tmux_title = get(g:, 'R_tmux_title', 'NvimR')
...
function! StartR_ExternalTerm(rcmd)
    ...
    if g:Rcfg.rconsole_width > 0 && winwidth(0) > (g:Rcfg.rconsole_width + g:Rcfg.min_editor_width + 1 + (&number * &numberwidth))
        if g:Rcfg.rconsole_width == -1
            let tcmd .= "-h"
        else
            let tcmd .= "-h -l " . g:Rcfg.rconsole_width
        endif
    else
        let tcmd .= "-l " . g:Rcfg.rconsole_height
    endif
    ...
    if g:Rcfg.tmux_title != "automatic" && g:Rcfg.tmux_title != ""
        call system("tmux rename-window " . g:Rcfg.tmux_title)
    endif
