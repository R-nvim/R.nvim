...
function RSetPDFViewer()
    let g:rplugin.pdfviewer = tolower(g:Rcfg.pdfviewer)
    ...
    elseif g:rplugin.pdfviewer == "qpdfview"
        ...
        if !executable(g:Rcfg.pdfviewer)
            call RWarningMsg("R_pdfviewer (" . g:Rcfg.pdfviewer . ") not found.")
            return
        endif
        if g:Rcfg.synctex
            call RWarningMsg('Invalid value for R_pdfviewer: "' . g:Rcfg.pdfviewer . '" (SyncTeX will not work)')
        endif
    endif
    ...
    if !has("win32") && !g:rplugin.is_darwin && $WAYLAND_DISPLAY == ""
        if executable("wmctrl")
            ...
            if &filetype == "rnoweb" && g:Rcfg.synctex
...
if $XDG_CURRENT_DESKTOP == "sway"
    let g:Rcfg.openpdf = get(g:, "R_openpdf", 2)
elseif g:rplugin.is_darwin || $WAYLAND_DISPLAY != ""
    let g:Rcfg.openpdf = get(g:, "R_openpdf", 1)
else
    let g:Rcfg.openpdf = get(g:, "R_openpdf", 2)
endif

if g:rplugin.is_darwin
    let g:Rcfg.pdfviewer = "skim"
else
    if has("win32")
        let g:Rcfg.pdfviewer = "sumatra"
    else
        let g:Rcfg.pdfviewer = get(g:, "R_pdfviewer", "zathura")
    endif
endif
...
if g:rplugin.is_darwin
    ...
    let g:Rcfg.applescript = 0
endif

if &filetype == 'rnoweb'
    call RSetPDFViewer()
    call SetPDFdir()
    if g:Rcfg.synctex && $DISPLAY != "" && g:rplugin.pdfviewer == "evince"
