...
function CheckNvimcomVersion()
    ...
    if has_key(g:Rcfg, "remote_compldir")
        let scrptnm = g:Rcfg.remote_compldir . "/tmp/before_nrs.R"
    endif
...
function StartNServer()
    ...
    if has_key(g:Rcfg, "local_R_library_dir")
        let nrs_path = FindNCSpath(g:Rcfg.local_R_library_dir . '/nvimcom')
    else
    ...
    if g:Rcfg.objbr_opendf
        let $NVIMR_OPENDF = "TRUE"
    endif
    if g:Rcfg.objbr_openlist
...
function ListRLibsFromBuffer()
    if !has_key(g:Rcfg, "start_libs")
        let g:Rcfg.start_libs = "base,stats,graphics,grDevices,utils,methods"
    endif
    ...
    call map(lines, 'substitute(v:val, "\\", "", "g")')
    let libs = ""
    if len(g:Rcfg.start_libs) > 4
        let libs = '"' . substitute(g:Rcfg.start_libs, ",", '", "', "g") . '"'
    endif
    if len(lines) > 0
        if libs != ""
            let libs .= ", "
...
function s:BuildAllArgs(...)
    ...
    if has_key(g:Rcfg, "remote_compldir")
        let scrptnm = g:Rcfg.remote_compldir . "/tmp/build_args.R"
    endif
...
if exists("*WaitVimComStart")
    echohl WarningMsg
    call input("Please, uninstall Vim-R-plugin before using R-Nvim. [Press <Enter> to continue]")
    echohl None
endif
...
if len(s:ff) > 0
    function WarnDupNvimR()
        ...
        let msg = ["", "===   W A R N I N G   ===", "",
                    \ "It seems that Nvim-R is installed.",
                    \ "Please, remove it to avoid conflicts.",
                    \ "Below is the path of the installation:", ""]
        for ffd in ff
