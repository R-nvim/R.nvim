...
function RWeave(bibtex, knit, pdf)
    if !exists("s:check_latexcmd")
        ...
        if g:Rcfg.latexcmd[0] == "default"
            if !executable("xelatex")
                if executable("pdflatex")
                    let g:Rcfg.latexcmd = ['latexmk', '-pdf', '-pdflatex="pdflatex %O -file-line-error -interaction=nonstopmode -synctex=1 %S"']
                else
                    call RWarningMsg("You should install 'xelatex' to be able to compile pdf documents.")
                endif
            endif
            if (g:Rcfg.latexcmd[0] == "default" || g:Rcfg.latexcmd[0] == "latexmk") && !executable("latexmk")
                if executable("xelatex")
                    let g:Rcfg.latexcmd = ['xelatex', '-file-line-error', '-interaction=nonstopmode', '-synctex=1']
                elseif executable("pdflatex")
                    let g:Rcfg.latexcmd = ['pdflatex', '-file-line-error', '-interaction=nonstopmode', '-synctex=1']
                else
    ...
    if g:Rcfg.latexcmd[0] != "default"
        let pdfcmd = pdfcmd . ", latexcmd = '" . g:Rcfg.latexcmd[0] . "'"
        if len(g:Rcfg.latexcmd) == 1
            let pdfcmd = pdfcmd . ", latexargs = character()"
        else
            let pdfcmd = pdfcmd . ", latexargs = c('" . join(g:Rcfg.latexcmd[1:], "', '"). "')"
        endif
    endif
    ...
    if a:pdf == 0 || g:Rcfg.openpdf == 0 || b:pdf_is_open
        let pdfcmd = pdfcmd . ", view = FALSE"
    endif

    if a:pdf && g:Rcfg.openpdf == 1
        let b:pdf_is_open = 1
    endif

    if has_key(g:Rcfg, 'latex_build_dir')
        let pdfcmd .= ', builddir="' . g:Rcfg.latex_build_dir . '"'
    endif

    if a:knit == 0 && has_key(g:Rcfg, "sweaveargs")
        let pdfcmd = pdfcmd . ", " . g:Rcfg.sweaveargs
    endif
...
function SyncTeX_backward(fname, ln)
    ...
    if GoToBuf(rnwbn, rnwf, basedir, rnwln)
        ...
        elseif g:rplugin.has_awbt && has_key(g:Rcfg, 'term_title')
            call RRaiseWindow(g:Rcfg.term_title)
        endif
    endif
endfunction
...
function SyncTeX_forward(...)
    ...
    if !filereadable(b:rplugin_pdfdir . "/" . basenm . ".synctex.gz")
        ...
        if g:Rcfg.latexcmd != "default" && g:Rcfg.latexcmd !~ "synctex"
...
function SetPDFdir()
    ...
    if (g:Rcfg.latexcmd[0] =~ "default" || g:Rcfg.latexcmd[0] =~ "latexmk") && filereadable(expand("~/.latexmkrc"))
    ...
    if join(g:Rcfg.latexcmd) =~ "-outdir" || join(g:Rcfg.latexcmd) =~ "-output-directory"
        let b:rplugin_pdfdir = substitute(join(g:Rcfg.latexcmd), '.*\(-outdir\|-output-directory\)\s*=*\s*', '', '')
